...
import { getUser, getPlatform } from '../../utils'
export default class Cart extends Component {
  ...
  async Buy() {
    ...
    if (parseInt(price) > 13000) {
      ...
      Alert.alert(
        'Lo sentimos, el valor minimo para realizar un pedido',
        'son $13.000'
      )
    }
  }
     ...
     return (
      <FlatList
        ...
        renderItem = {(product) => (
          <View key = {product.item.id}>
            ...
            <View style = {[styles.btnFloat, styles.cant, {flexDirection: 'column', alignItems: 'center', justifyContent: 'center'}]}>
              <Text style = {[styles.colorWhite, ]}> {product.item.cant} </Text>
            </View>
      ...
      return (
        <View style = {styles.noProducts}>
          <Text style = {{color: 'white', fontSize: 18}}> Aun no tienes productos en el carrito </Text>
          <TouchableHighlight onPress = {() => this.backToShop}>
            <Text style = {{color: 'blue'}}> Compra algo </Text>
          </TouchableHighlight>
        </View>
      )
    }
...
const styles = StyleSheet.create({
  ...
  colorWhite: {
    ...
    fontFamily:  getPlatform ===  'android'? 'Typo_Round_Regular_Demo': 'Typo Round Regular Demo'
  },
  list: {
    alignItems: 'center'
  },
